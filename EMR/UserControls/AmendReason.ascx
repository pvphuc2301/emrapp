...
<script>
    if (document.getElementById('<%= amendReasonBox.ClientID %>').parentElement.id != "AmendReasonPlaceHolder") {
        console.error("amend reason must place inside a div id = AmendReasonPlaceHolder");
    }
    else if (document.getElementById("btnComplete")) {
        console.error("missing button complete id = btnComplete")
    }
    else {
